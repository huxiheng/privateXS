...
#define kThumbnailLength    (([UIScreen mainScreen].bounds.size.width - 8)/4.0f)
...
@interface HYAssetPickerController ()
...
@implementation HYVideoTitleView
...
-(void)drawRect:(CGRect)rect{
    ...
    UIImage *videoIcon=[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"HYAssetPicker.Bundle/Images/AssetsPickerVideo"]];
...
@interface HYTapAssetView ()

@property(nonatomic,retain)UIImageView *selectView;

@end

@implementation HYTapAssetView
...
+ (void)initialize
{
    checkedIcon     = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"HYAssetPicker.Bundle/Images/%@",(!IS_IOS7) ? @"AssetsPickerChecked" : @"AssetsPickerChecked"]]];
    
    checkedNoIcon     = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"HYAssetPicker.Bundle/Images/%@",(!IS_IOS7) ? @"No" : @"No"]]];
...
@interface HYAssetView ()<HYTapAssetViewDelegate>

@property (nonatomic, strong) ALAsset *asset;

@property (nonatomic, weak) id<HYAssetViewDelegate> delegate;

@property (nonatomic, retain) UIImageView *imageView;
@property (nonatomic, retain) HYVideoTitleView *videoTitle;
@property (nonatomic, retain) HYTapAssetView *tapAssetView;

@end

@implementation HYAssetView
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        _videoTitle=[[HYVideoTitleView alloc] initWithFrame:CGRectMake(0, kThumbnailSize.height-20, kThumbnailSize.width, titleHeight)];
        ...
        _tapAssetView=[[HYTapAssetView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
...
#pragma mark - HYTapAssetView Delegate
...
@interface HYAssetViewCell ()<HYAssetViewDelegate>

@end

@class HYAssetViewController;

@implementation HYAssetViewCell
...
- (void)bind:(NSArray *)assets selectionFilter:(NSPredicate*)selectionFilter minimumInteritemSpacing:(float)minimumInteritemSpacing minimumLineSpacing:(float)minimumLineSpacing columns:(int)columns assetViewX:(float)assetViewX{
    //照片应该从左边往右边设置assetViewX的值为1
    assetViewX = 1;
    if (self.contentView.subviews.count<assets.count) {
        for (int i=0; i<assets.count; i++) {
            if (i>((NSInteger)self.contentView.subviews.count-1)) {
                HYAssetView *assetView=[[HYAssetView alloc] initWithFrame:CGRectMake(assetViewX+(kThumbnailSize.width+minimumInteritemSpacing)*i, minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height)];
                [assetView bind:assets[i] selectionFilter:selectionFilter isSeleced:[((HYAssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i]]];
                assetView.delegate=self;
                [self.contentView addSubview:assetView];
            }
            else{
                ((HYAssetView*)self.contentView.subviews[i]).frame=CGRectMake(assetViewX+(kThumbnailSize.width+minimumInteritemSpacing)*(i), minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height);
                [(HYAssetView*)self.contentView.subviews[i] bind:assets[i] selectionFilter:selectionFilter isSeleced:[((HYAssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i]]];
            }
        ...
        for (NSInteger i=self.contentView.subviews.count; i>0; i--) {
            if (i>assets.count) {
                [((HYAssetView*)self.contentView.subviews[i-1]) removeFromSuperview];
            }
            else{
                ((HYAssetView*)self.contentView.subviews[i-1]).frame=CGRectMake(assetViewX+(kThumbnailSize.width+minimumInteritemSpacing)*(i-1), minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height);
                [(HYAssetView*)self.contentView.subviews[i-1] bind:assets[i-1] selectionFilter:selectionFilter isSeleced:[((HYAssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i-1]]];
            }
        }
    }
}

#pragma mark - HYAssetView Delegate
...
@interface HYAssetViewController ()<HYAssetViewCellDelegate>{
    int columns;
...
@implementation HYAssetViewController
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    [self updateBottomView:self.indexPathsForSelectedItems.count];
}
...
{
    //用xib文件
    self.bottomView = [[[NSBundle mainBundle] loadNibNamed:@"chooseView" owner:self options:nil] lastObject];
    self.bottomView.translatesAutoresizingMaskIntoConstraints = NO;
    [self.view.window addSubview:self.bottomView];
    
    self.preBtn.layer.cornerRadius = 5.0f;
    self.confirmBtn.layer.cornerRadius = 5.0f;
    
    NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:self.bottomView
                                                                  attribute:NSLayoutAttributeLeading
                                                                  relatedBy:NSLayoutRelationEqual
                                                                     toItem:self.view.window
                                                                  attribute:NSLayoutAttributeLeading
                                                                 multiplier:1.0f
                                                                   constant:0.0f];
    [self.view.window addConstraint:constraint];

    constraint = [NSLayoutConstraint constraintWithItem:self.bottomView
...
-(void)updateBottomView:(NSInteger)selectCount
{
    if (selectCount != 0) {
        self.preBtn.enabled = YES;
        self.preBtn.backgroundColor = [UIColor colorWithHexString:@"#fbfefb"];
        self.preBtn.layer.borderColor = [UIColor colorWithHexString:@"#cbcbcb"].CGColor;
        self.preBtn.layer.borderWidth = 0.3f;
        [self.preBtn setTitleColor:[UIColor colorWithHexString:knavBJColor] forState:UIControlStateNormal];
        
        self.confirmBtn.enabled = YES;
        self.confirmBtn.backgroundColor = [UIColor colorWithHexString:knavBJColor];
        self.confirmBtn.layer.borderColor = [UIColor colorWithHexString:@"#299eca"].CGColor;
        self.confirmBtn.layer.borderWidth = 0.3f;
        [self.confirmBtn setTitle:[NSString stringWithFormat:@"确定(%ld)",selectCount] forState:UIControlStateNormal];
    ...
    else
    {
        ...
        self.preBtn.backgroundColor = [UIColor colorWithHexString:@"#eaebed"];
        self.preBtn.layer.borderColor = [UIColor clearColor].CGColor;
        [self.preBtn setTitleColor:[UIColor colorWithHexString:@"#cfd0d2"] forState:UIControlStateNormal];
        
        self.confirmBtn.enabled = NO;
        self.confirmBtn.backgroundColor = [UIColor colorWithHexString:@"#eaebed"];
        [self.confirmBtn setTitle:@"确定" forState:UIControlStateNormal];
        self.confirmBtn.layer.borderColor = [UIColor clearColor].CGColor;
        [self.confirmBtn setTitleColor:[UIColor colorWithHexString:@"#cfd0d2"] forState:UIControlStateNormal];
    }
}
//预览
-(IBAction)previewSelectPicture:(id)sender
{
    PZViewController *bigViewController = [[PZViewController alloc] init];
    bigViewController.arrayOK = self.indexPathsForSelectedItems;
    [self.navigationController pushViewController:bigViewController animated:YES];
    
}
//确定
-(IBAction)confirmSelectPicture:(id)sender
{
    HYAssetPickerController *pickerController = (HYAssetPickerController *)self.navigationController;
    [pickerController.delegate getAssetFromPickerController:self.indexPathsForSelectedItems];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    HYAssetPickerController *picker = (HYAssetPickerController *)self.navigationController;
    
    HYAssetViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell==nil) {
        cell=[[HYAssetViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
    }
    cell.delegate=self;
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    //第一个1是为了在UITableView的底部，显示有多少张照片
    //第二个1是为了在提供空间显示bottomView，加在window上面的
    return ceil(self.assets.count*1.0/columns)+1 + 1;
}
...
#pragma mark - HYAssetViewCell Delegate

- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    HYAssetPickerController *vc = (HYAssetPickerController *)self.navigationController;
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    [self updateBottomView:_indexPathsForSelectedItems.count];
    ...
    NSLog(@"self.number is %ld",self.number);
}
...
@interface HYAssetGroupViewCell ()

@property (nonatomic, strong) ALAssetsGroup *assetsGroup;

@end

@implementation HYAssetGroupViewCell
...
@interface HYAssetGroupViewController()
...
@implementation HYAssetGroupViewController
...
- (void)showNotAllowed
{
    ...
    UIImageView *padlock    = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"HYAssetPicker.Bundle/Images/AssetsPickerLocked"]]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HYAssetGroupViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil)
    {
        cell = [[HYAssetGroupViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
    }
...
{
    HYAssetViewController *vc = [[HYAssetViewController alloc] init];
...
@implementation HYAssetPickerController

- (id)init
{
    HYAssetGroupViewController *groupViewController = [[HYAssetGroupViewController alloc] init];
...
- (void)viewDidLoad
{
    ...
    [self.navigationBar setBarTintColor:[UIColor colorWithHexString:knavBJColor]];
